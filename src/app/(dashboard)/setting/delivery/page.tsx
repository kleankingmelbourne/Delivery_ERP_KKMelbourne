...
import { 
  Search, 
  Truck, // [변경] 아이콘 변경 (UserCircle -> Truck)
  Loader2, 
  CheckCircle2, 
  AlertCircle,
  ArrowRightLeft,
...
interface Customer {
  id: string;
  name: string;
  in_charge_delivery: string | null; // [변경] 컬럼명 변경
}
...
export default function DeliveryInchargePage() {
  ...
  const [drivers, setDrivers] = useState<DeliveryDriver[]>([]); // [변경] salesReps -> drivers
  const [loading, setLoading] = useState(true);

  // Filter & Search State
  const [searchTerm, setSearchTerm] = useState("");
  const [filterDriverId, setFilterDriverId] = useState<string>("all"); // [변경] 변수명 변경

  // Bulk Action State
  const [bulkTargetDriverId, setBulkTargetDriverId] = useState<string>(""); // [변경] 변수명 변경
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        // 1) Drivers (Profiles where status = active)
        ...
        if (profileData) setDrivers(profileData);

        // 2) Customers
        const { data: customerData } = await supabase
          .from("customers")
          .select("id, name, in_charge_delivery") // [변경] 컬럼명 변경
          .order("name");
  ...
  const filteredCustomers = customers.filter((c) => {
    ...
    if (filterDriverId === "all") matchesDriver = true;
    else if (filterDriverId === "unassigned") matchesDriver = c.in_charge_delivery === null;
    else matchesDriver = c.in_charge_delivery === filterDriverId;

    return matchesSearch && matchesDriver;
  });

  const unassignedCount = customers.filter(c => c.in_charge_delivery === null).length;

  // 3. 개별 변경 (Inline Edit)
  const handleDriverChange = async (customerId: string, newDriverId: string) => {
    setUpdatingId(customerId);
    setUpdateSuccessId(null);
    const valueToSave = newDriverId === "" ? null : newDriverId;

    try {
      const { error } = await supabase
        .from("customers")
        .update({ in_charge_delivery: valueToSave }) // [변경] 컬럼명 변경
        .eq("id", customerId);

      if (error) throw error;

      setCustomers((prev) => 
        prev.map((c) => c.id === customerId ? { ...c, in_charge_delivery: valueToSave } : c)
      );
  ...
  const handleBulkUpdate = async () => {
    if (!bulkTargetDriverId) return alert("Please select a target Driver.");
    if (filteredCustomers.length === 0) return alert("No customers to update.");

    const targetName = drivers.find(d => d.id === bulkTargetDriverId)?.display_name || "Unassigned";
    const confirmMsg = `Are you sure you want to assign ${filteredCustomers.length} customers to "${targetName}"?`;
    
    if (!window.confirm(confirmMsg)) return;

    setIsBulkUpdating(true);
    const valueToSave = bulkTargetDriverId === "unassigned" ? null : bulkTargetDriverId;
    ...
    try {
      ...
      setCustomers(prev => 
        prev.map(c => targetIds.includes(c.id) ? { ...c, in_charge_delivery: valueToSave } : c)
      );

      alert("Bulk update successful!");
      setBulkTargetDriverId("");
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 flex items-center gap-2">
            <Truck className="w-7 h-7 text-emerald-600" /> {/* [변경] 아이콘 색상 변경 */}
            Set Delivery Incharge
          </h1>
          <p className="text-sm text-slate-500 mt-1">
            Manage and re-assign delivery drivers.
          </p>
        </div>
        ...
        <div className="flex flex-col xl:flex-row gap-4 items-end xl:items-center justify-between">
              ...
              <div className="relative">
                ...
                >
                  ...
                  <optgroup label="Drivers">
                    {drivers.map(drv => (
                      <option key={drv.id} value={drv.id}>{drv.display_name}</option>
                    ))}
                  </optgroup>
                </select>
          ...
          <div className="flex items-end gap-3 w-full xl:w-auto bg-slate-50 p-3 rounded-lg border border-slate-200">
            <div className="space-y-1.5 flex-1 w-full sm:w-64">
              <label className="text-xs font-bold text-emerald-600 ml-1">2. Assign filtered ({filteredCustomers.length}) to:</label>
              <div className="relative">
                <select
                  className="w-full h-10 pl-3 pr-10 rounded-md border border-emerald-200 bg-white text-sm focus:ring-2 focus:ring-emerald-500 outline-none appearance-none transition-shadow"
                  value={bulkTargetDriverId}
                  onChange={(e) => setBulkTargetDriverId(e.target.value)}
                >
            ...
            <Button 
              ...
              disabled={isBulkUpdating || filteredCustomers.length === 0 || !bulkTargetDriverId}
              className="bg-emerald-600 hover:bg-emerald-700 text-white font-bold h-10 w-32 shrink-0 shadow-sm"
            >
          ...
          <table className="w-full text-sm text-left">
            <thead className="bg-slate-50 border-b border-slate-200 text-slate-500 uppercase font-semibold text-xs">
              <tr>
                ...
                <th className="px-6 py-3 w-[40%]">Assigned Driver</th>
            ...
            <tbody className="divide-y divide-slate-100 bg-white">
              {loading ? (
                ...
                filteredCustomers.map((customer) => {
                  ...
                  const isUnassigned = !customer.in_charge_delivery;
                    ...
                    <tr key={customer.id} className={cn("hover:bg-slate-50 transition-colors", isUnassigned && "bg-red-50/30")}>
                      ...
                      <td className="px-6 py-3">
                        <div className="relative">
                          <select
                            className={cn(
                              "w-full max-w-xs h-10 pl-3 pr-10 rounded-md border text-sm appearance-none outline-none focus:ring-2 focus:ring-emerald-500 transition-all cursor-pointer font-medium",
                              isUnassigned 
                            ...
                            value={customer.in_charge_delivery || ""}
                            onChange={(e) => handleDriverChange(customer.id, e.target.value)}
                            disabled={isUpdating}
                          >
                      ...
                      <td className="px-6 py-3 text-center">
                        {isUpdating && (
                          <div className="flex items-center justify-center text-emerald-600 text-xs font-medium animate-pulse">
